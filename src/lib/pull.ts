// Gemeinsame Pull-Logik – genutzt von /api/admin/pull (manuell) und /api/cron/pull (Zeitplan).
...
export async function runPull(admin: any, customer_id: string | null) {
  // ASINs laden
  ...
  if (!asins.length) return { asins_done: 0, errors: ["Keine ASINs gefunden"] };
  ...
  for (let i = 0; i < asins.length; i += BATCH) {
    ...
    try {
      ...
      for (const p of products) {
        ...
        if (bbHist.length >= 2) {
          ...
          for (let j = 0; j < bbHist.length - 1; j += 2) {
            ...
            bbRows.push({
              asin, ts: kmToDate(km).toISOString(), ts_km: km, seller_id: sid,
        ...
        if (prSeries && prSeries.length >= 3) {
          ...
          for (let j = 0; j < prSeries.length - 2; j += 3) {
            const km = prSeries[j];
            const eur = combineBuyBoxPrice(prSeries[j + 1], prSeries[j + 2]);
      ...
      if (logId) await admin.from("pull_log").update({ asins_done: done }).eq("id", logId);
    } catch (e: any) {
